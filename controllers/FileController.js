...
const avatarPath = process.cwd() + '\\uploads\\avatar\\';
...
export const getAvatar = async (request,response)=>{
    let fileName = request.query.fileName;
    let canGetAll = false;
    console.log(fileName + " requested avatar");
    //0차. 권한 확인
    if(checkAdminAuth(request.session.auth)){
        canGetAll=true;
    }else if(checkUserAuth(request.session.auth)){
    ...
    //2차. 파일이 있는가?
    if(!fs.existsSync(avatarPath + fileName)){
        response.status(404);
        return;
    }
    //3차. 파일을 전송하자
    //4차. 오류가 발생하면 오류를 전송해주자
    response.sendFile(avatarPath+fileName,(Errback)=>{
       if(Errback){
            response.status(404);
       }
    });
